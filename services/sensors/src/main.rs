...
impl Quaternion {
    ...
    pub fn normalize(&self) -> Self {
        let n = (self.w*self.w + self.x*self.x + self.y*self.y + self.z*self.z).sqrt().max(1e-10);
...
impl ComplementaryFilter {
    pub fn new(alpha: f32) -> Self {
        Self { orientation: Quaternion::identity(), alpha }
    }
    ...
    pub fn update(&mut self, gyro: &Vec3, accel: &Vec3, dt: f32) -> &Quaternion {
        ...
        if angle > 1e-6 {
            let axis = Vec3::new(gyro.x / (angle/dt), gyro.y / (angle/dt), gyro.z / (angle/dt));
        ...
        if g > 9.0 && g < 10.6 {
            ...
            let gravity_est = Vec3::new(
                2.0*(self.orientation.x*self.orientation.z - self.orientation.w*self.orientation.y),
                2.0*(self.orientation.w*self.orientation.x + self.orientation.y*self.orientation.z),
                self.orientation.w*self.orientation.w - self.orientation.x*self.orientation.x
                    - self.orientation.y*self.orientation.y + self.orientation.z*self.orientation.z,
            );
            let error = Vec3::new(
...
impl SensorDaemon {
    ...
    fn classify_orientation(&self, accel: &Vec3) -> ScreenOrientation {
        ...
        if accel.y.abs() > threshold {
            if accel.y > 0.0 { ScreenOrientation::ReversePortrait } else { ScreenOrientation::Portrait }
        } else if accel.x.abs() > threshold {
            if accel.x > 0.0 { ScreenOrientation::Landscape } else { ScreenOrientation::ReverseLandscape }
        } else {
